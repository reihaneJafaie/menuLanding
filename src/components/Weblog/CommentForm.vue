<template>
  <section class="mt-16">
    ...
    <form @submit.prevent="sendFormBlog" id="comment" action="" class="w-full">
      ...
      <textarea
        ...
        :class="{ 'showError ': !body && showErrorBody }"
...
<script setup>
import { ref } from "vue";
import { useFormWeblog } from "@/stores/formWeblog.js";
import Swal from "sweetalert2";
let store = useFormWeblog();
let nameRef = ref("");
let emailRef = ref("");
let bodyRef = ref("");
...
function sendFormBlog() {
  ...
  console.log(fd);
  store
    .getFormWeblogFromServer(fd, props.idPost)
    .then((res) => {
      if (res.data.success) {
        Swal.fire({
          icon: "success",
